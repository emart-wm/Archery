...
def notify_for_audit(audit_id, **kwargs):
    ...
    if workflow_type == WorkflowDict.workflow_type['query']:
        ...
        workflow_content = '''【SRC Table Name】：{}\n【Mode】：{}\n【Condition】：{}\n'''.format(
            workflow_detail.src_table_name,
            workflow_detail.mode,
            workflow_detail.condition)
    else:
        raise Exception('Workflow Type is not Correct。工单类型不正确！')

    # 准备消息格式
    if status == WorkflowDict.workflow_status['audit_wait']:  # 申请阶段
        msg_title = "[{}]New WorkFlow Application#{}".format(workflow_type_display, audit_id)
        ...
        msg_cc = Users.objects.filter(username__in=kwargs.get('cc_users', []))
        # 消息内容
        msg_content = '''【ApplyTime】：{}\n【Instance】：{}\n【DB】：{}\n【WorkFlow Title】：{}\n【WebLink】：{}\n【Rejected Reason】：{}\n【Tips】：this Workflow is not approved. Please modify it according to the rejection reason! '''.format(
            workflow_detail.create_time.strftime('%Y-%m-%d %H:%M:%S'),
            instance,
            db_name,
            workflow_title,
        ...
        msg_title = "[{}]The Applicant terminates WorkFlow voluntarily#{}".format(workflow_type_display, audit_id)
        # 接收人，发送给该资源组内对应权限组所有的用户
...
def notify_for_execute(workflow):
    ...
    msg_title = "[{}]WorkFlow{}#{}".format(WorkflowDict.workflow_type['sqlreview_display'],
    ...
    if sys_config.get('ddl_notify_auth_group') and workflow.status == 'workflow_finish':
        ...
        if workflow.syntax_type == 1:
            # 消息内容通知
            msg_title = '[AscendEX SQL]New DDL Statement has been executed.#{}'.format(audit_id)
            msg_content = '''【Applicant】：{}\n【Resource Group】：{}\n【Instance】：{}\n【DB】：{}\n【WorkFlow Title】：{}\n【WebLink】：{}\n【WorkFlow Detail】：{}\n'''.format(
